...
The EZTest Selenium Java SDK allows you to automatically sync your Selenium test executions with EZTest test management system. The SDK:

> 👥 **For Users**: See [USER_GUIDE.md](USER_GUIDE.md) - Complete step-by-step guide to connect your tests!  
> 🔑 **Get JWT Token**: See [GET_JWT_TOKEN.md](GET_JWT_TOKEN.md) - Visual guide to extract JWT token from browser!  
> 🆔 **Get Project ID**: See [GET_PROJECT_ID.md](GET_PROJECT_ID.md) - How to find your Project ID from URL or settings!  
> 📋 **JSON Format**: See [JSON_FORMAT_SPECIFICATION.md](JSON_FORMAT_SPECIFICATION.md) - Complete JSON report format specification!  
> 📥 **Import JSON**: See [HOW_TO_IMPORT_JSON.md](HOW_TO_IMPORT_JSON.md) - Step-by-step guide to import JSON reports!  
> 📖 **For detailed usage instructions, see [USAGE_GUIDE.md](USAGE_GUIDE.md)**  
> 🧪 **For testing instructions, see [TESTING_GUIDE.md](TESTING_GUIDE.md)**  
> 🔧 **For environment variables, see [ENVIRONMENT_VARIABLES.md](ENVIRONMENT_VARIABLES.md)**  
> 🔑 **API Key Setup**: See [API_KEY_SETUP.md](API_KEY_SETUP.md) for all ways to set your API key!  
> 🎯 **Multiple Projects Support**: See [MULTIPLE_PROJECTS.md](MULTIPLE_PROJECTS.md) for configuring default project and using multiple projects!  
> ⚡ **Quick Examples**: See [QUICK_EXAMPLES.md](QUICK_EXAMPLES.md) for quick reference on choosing specific projects!  
> ⚙️ **Configuration Comparison**: See [CONFIGURATION_COMPARISON.md](CONFIGURATION_COMPARISON.md) to understand when environment variables are needed!  
> 📘 **Simple Example**: See [SIMPLE_EXAMPLE.md](SIMPLE_EXAMPLE.md) for a beginner-friendly explanation of how projects work!
...
2. **API Key** - JWT token from EZTest login (extract from browser session or login via API)
...
The SDK supports importing test reports from JSON format (e.g., ExtentSparkReporter) and syncing them with EZTest.

> 📋 **For complete JSON format specification, see [JSON_FORMAT_SPECIFICATION.md](JSON_FORMAT_SPECIFICATION.md)**

### Supported JSON Format

The SDK accepts JSON reports in **ExtentSparkReporter-compatible format**. The JSON structure is:
...
**Sample Files:**
- Sample JSON report: [samples/sample-report.json](samples/sample-report.json)
...
1. **Test Cases**: Creates or maps test cases in EZTest for each test result
   - **Smart Lookup**: First tries to find existing test case by `test_id`, then by title
   - **If found**: Uses existing test case and updates execution history
   - **If not found**: Creates new test case with:
     - Title: `test_id + " - " + test_name` (or just `test_name` if `test_id` is not available)
     - Description: Includes class name, tags, and environment info
     - Priority: HIGH if "critical" or "high" tags are present, otherwise MEDIUM
     - Status: ACTIVE

2. **Test Run**: Creates a test run with:
   - Name built from build_id, execution_profile, and timestamp
   - Environment string built from browser and environment_url
   - Description includes report source and triggered_by information

3. **Test Results**: Records all test results with:
   - Status (PASS/FAIL/SKIP mapped to PASSED/FAILED/SKIPPED)
