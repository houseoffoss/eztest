...
const ButtonOrange = React.forwardRef<HTMLButtonElement, ButtonOrangeProps>(
  ({ className, variant = "default", size = "default", onClick, ...props }, ref) => {
    ...
    // Glass morphism style with orange gradient
    if (variant === "default") {
      return (
        <div
    ...
    if (variant === "light") {
      return (
        <button
          ...
          style={{
            backgroundColor: 'rgba(217, 127, 76, 0.1)',
            backdropFilter: 'blur(40px)',
            border: '1px solid rgba(217, 127, 76, 0.3)',
            color: '#D97F4C',
            boxShadow: '0 10px 30px -12px rgba(0, 0, 0, 0.6)',
          }}
          onClick={onClick}
          ...
          onMouseEnter={(e) => {
            const target = e.currentTarget as HTMLElement;
            target.style.backgroundColor = 'rgba(217, 127, 76, 0.2)';
            target.style.boxShadow = '0 18px 45px -18px rgba(217, 127, 76, 0.4)';
            target.style.transform = 'translateY(-1px)';
          }}
          onMouseLeave={(e) => {
            const target = e.currentTarget as HTMLElement;
            target.style.backgroundColor = 'rgba(217, 127, 76, 0.1)';
            target.style.boxShadow = '0 10px 30px -12px rgba(0, 0, 0, 0.6)';
            target.style.transform = 'translateY(0)';
          }}
          {...props}
